import unittest
import pandas as pd
# Visualizer functions and utils for color formatting
from mapvis import visualizer, utils
import os
import shutil
...
class TestCelltypeMappingTable(unittest.TestCase):
    ...
    def setUp(self):
        self.mapping1 = {'Tcell_1': 'T-cell',
                         'Bcell_A': 'B-cell', 'Macro_X': 'Myeloid'}
        self.mapping2 = {'T_cell_X': 'T-cell', 'B_cell_Y': 'B-cell',
                         'Mono_1': 'Myeloid', 'NK_1': 'NK cell'}
        self.custom_colors = {'T-cell': '#FF0000',
                              'B-cell': '#00FF00', 'Myeloid': '#0000FF'}  # Base hex
    ...
    def test_empty_mappings(self):
        ...
        # Or empty string, depending on impl; current utils.get_legend_html returns None
    ...
    def test_no_common_consensus(self):
        ...
        html_table, html_legend, df = visualizer.create_celltype_mapping_table(
            map1, map2)
        # Each label forms its own consensus group
    ...
    def test_legend_generation(self):
        _, html_legend_true, _ = visualizer.create_celltype_mapping_table(
            self.mapping1, self.mapping2, show_legend=True)
        self.assertIsNotNone(html_legend_true)
        self.assertIn("Legend", html_legend_true)

        _, html_legend_false, _ = visualizer.create_celltype_mapping_table(
            self.mapping1, self.mapping2, show_legend=False)
    ...
    def test_custom_colors_in_output(self):
        ...
        tcell_legend_swatch = utils.format_color_with_opacity(
            tcell_color_base, 1.0)  # ff0000ff
        tcell_table_cell = utils.format_color_with_opacity(
            tcell_color_base, 0.5)   # #ff000080
        ...
        self.assertIn(
            f"background-color: {tcell_legend_swatch}", html_legend.lower())
        self.assertIn(
            f"background-color: {tcell_table_cell}", html_table.lower())

    def test_caption_in_table(self):
        caption = "My Custom Celltype Table"
        html_table, _, _ = visualizer.create_celltype_mapping_table(
...
class TestFeatureMappingTable(unittest.TestCase):
    ...
    def test_smoke_test_feature(self):
        html_table, html_legend, df = visualizer.create_feature_mapping_table(
            self.feature_df)
        self.assertIsInstance(html_table, str)
        # Can be None if no valid consensus labels for legend
        self.assertIsInstance(html_legend, str) 
        self.assertIsInstance(df, pd.DataFrame)
        self.assertTrue(len(html_table) > 0)
    ...
    def test_empty_dataframe_feature(self):
        empty_df = pd.DataFrame(
            columns=[self.default_protein_col, self.default_rna_col])
        html_table, html_legend, df = visualizer.create_feature_mapping_table(
            empty_df)
        self.assertIn("No data to display", html_table)
        self.assertIsNone(html_legend)
    ...
    def test_df_columns_feature(self):
        ...
        expected_cols = ["Protein name", "Operation",
                         "Consensus label", "Operation", "RNA name"]
    ...
    def test_one_to_one_mapping(self):
        ...
        self.assertEqual(df_res.iloc[0]["Operation"].iloc[0], "")
        # Check second "Operation"
        self.assertEqual(df_res.iloc[0]["Operation"].iloc[1], "")
    ...
    def test_one_to_many_mapping(self):  # P3 -> R3a/R3b
        df_input = pd.DataFrame(
            {'Protein name': ['P3'], 'RNA name': ['R3a/R3b']})
        _, _, df_res = visualizer.create_feature_mapping_table(df_input)
    ...
    def test_many_to_one_mapping(self):  # P2a/P2b -> R2
        df_input = pd.DataFrame(
            {'Protein name': ['P2a/P2b'], 'RNA name': ['R2']})
        _, _, df_res = visualizer.create_feature_mapping_table(df_input)
    ...
    def test_many_to_many_mapping(self):  # P4a/P4b -> R4a/R4b
        df_input = pd.DataFrame(
            {'Protein name': ['P4a/P4b'], 'RNA name': ['R4a/R4b']})
        _, _, df_res = visualizer.create_feature_mapping_table(df_input)
        self.assertEqual(len(df_res), 2)
    ...
    def test_legend_generation_feature(self):
        _, html_legend_true, _ = visualizer.create_feature_mapping_table(
            self.feature_df, show_legend=True)
        self.assertIsNotNone(html_legend_true)
        self.assertIn("Legend", html_legend_true)

        _, html_legend_false, _ = visualizer.create_feature_mapping_table(
            self.feature_df, show_legend=False)
        self.assertIsNone(html_legend_false)

    def test_custom_colors_in_output_feature(self):
        # R1: '#FF0000' -> #ff0000
        r1_color_base = self.custom_colors_feat['R1'].lower()
        r1_legend_swatch = utils.format_color_with_opacity(
            r1_color_base, 1.0)  # ff0000ff
        r1_table_cell = utils.format_color_with_opacity(
            r1_color_base, 0.5)   # #ff000080

        html_table, html_legend, _ = visualizer.create_feature_mapping_table(
            self.feature_df, color_scheme=self.custom_colors_feat
        )
        self.assertIn(
            f"background-color: {r1_legend_swatch}", html_legend.lower())
    ...
    def test_caption_in_table_feature(self):
        ...
        html_table, _, _ = visualizer.create_feature_mapping_table(
            self.feature_df, caption=caption)
        self.assertIn(f"<caption>{caption}</caption>", html_table)
...
# Note: The test_one_to_one_mapping had a line:
# self.assertEqual(df_res.iloc[0]["Operation"], "")
# If "Operation" is a duplicated column name (as per test_df_columns_feature),
# df_res.iloc[0]["Operation"] would return a Series. Comparing a Series to "" directly
# causes `ValueError: The truth value of a Series is ambiguous`.
